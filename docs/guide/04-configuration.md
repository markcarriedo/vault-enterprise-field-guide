...
2. Added `terraform/vault-config/dynamic-secrets.tf`: a new, narrowly-scoped IAM role
   ...
   role; an inline policy granting that existing role `sts:AssumeRole` on the new one; the
   ...
   `vault_aws_secret_backend_role` named `field-guide-app`, `role_arns` pointing at the new
   IAM role, `default_sts_ttl = 900`.
3. Extended the `field-guide-app` **policy** (the same one used for KV v2 and AWS auth) with
   `read` on `aws/creds/field-guide-app` — one app identity, scoped across multiple secrets
...
4. **Verified with real AWS credentials, not just a successful `vault read`** — from the same
   SSM session used for the AWS auth test: logged in via AWS auth, read
   `aws/creds/field-guide-app` (got back an access key, secret key, session token, and a
   ...
   (`assumed-role/vault-dynamic-demo/...`), not the Vault node's own role — proof the
